# proto-agent — AI Agent App Starter Kit
...
find . -type f \( -name "*.json" -o -name "*.ts" -o -name "*.tsx" \) \
  | xargs sed -i '' 's/proto-agent/my-agent/g'
...
```typescript
import { createLinearGraph, createLoopGraph, createBranchingGraph } from 'proto-agent-flows';
...
```typescript
import { PromptLoader, PromptRegistry } from 'proto-agent-prompts';
