...
The `packages/mcp` package runs a lightweight Node.js server that speaks the MCP protocol over stdio. It imports tools from `proto-agent-tools` and exposes them to any MCP client.
...
{
  "bin": {
    "proto-agent-mcp": "./dist/index.js"
  }
}
```

After replacing `proto-agent` with your actual project name, you can publish the package to npm and install it globally:
